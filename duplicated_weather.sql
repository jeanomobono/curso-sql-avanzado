...
select trunc(SAMPLED,'mm') mth, sum(RAINFALL) from PERTH_WEATHER GROUP by trunc(SAMPLED,'mm') order by 2 desc;

select sampled, rowid, 
row_number() over (partition by sampled order by rowid) as dup_cnt
from perth_weather;
